<template>
  ...
  >
    ...
    <p>
      Without collapsing, the search results will return all documents that match the query. In this case, the number is
      270 documents.
    </p>
    <CodeOutput :code-output="codeOutput1" />

    <p>Let's look at the count of documents that matched the query per year in the <InlineCode text="apod index" />.</p>
    ...
    <p>
      Collapsing search results by year will return only one document per year that matches the query. That returned
      document will be the one with the highest <InlineCode text="_score" /> for that year.
    </p>
    ...
    <p>
      We confirm that the first hit from 2024 has a <InlineCode text="_score" /> of <InlineCode text="7.789091" />,
      which is the same as the one in the response with collapsing.
    </p>
    ...
    <p>
      Expanding collapsed results allows you to retrieve more than one document per year that matches the query. Control
      how documents are sorted within each collapsed group, and more.
    </p>
    <p>
    ...
    <p>
      The documents are sorted by <InlineCode text="_score" /> within each collapsed group. They also match the scores
      in the response without collapsing.
    </p>
    ...
    <p>
      ...
      <InlineCode text="search_after" /> parameter to paginate through the results. This is useful when you want to
      retrieve all collapsed results without missing any.
    </p>
    ...
    <p>Let's create a new index where we are going to create 40 000 documents. Each user, will have 2 documents.</p>
    ...
    <p>
      Now we are ready to use <InlineCode text="search_after" /> to paginate through the collapsed results. Since we
      have 2 documents per user, we can expect to have 20 000 collapsed results.
    </p>
    <CodeBlock :code="pythonCodeSnippet15" language="python" @show-toast="handleShowToast" />
    <CodeOutput :code-output="codeOutput12" />

    <p>
      We see that the last user ID in the collapsed results is <InlineCode text="19999" /> and the number of collapsed
      hits is <InlineCode text="20000" />, which is what we expected.
    </p>
